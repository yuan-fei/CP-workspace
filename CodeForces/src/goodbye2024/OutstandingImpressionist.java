...
public class OutstandingImpressionist {
	public static void main(String[] args) {
		...
		for (int i = 0; i < t; i++) {
			int n = in.nextInt();
			int[][] arr = getIntArr(in, n, 2);

			String r = solve(n, arr);
	...
	private static String solve(int n, int[][] arr) {
		...
		Map<Integer, Integer> fixedToCnt = new HashMap<>();
		for (int i = 0; i < n; i++) {
			if (arr[i][0] == arr[i][1]) {
				cnt[arr[i][0] + 1] = 1;
				fixedToCnt.put(arr[i][0], fixedToCnt.getOrDefault(arr[i][0], 0) + 1);
			}
		}
		...
		for (int i = 0; i < n; i++) {
			if (arr[i][0] == arr[i][1]) {
				if (fixedToCnt.get(arr[i][0]) > 1) {
					sb.append('0');
				} else {
					sb.append('1');
				}
			} else {
				if (cnt[arr[i][1] + 1] - cnt[arr[i][0]] == arr[i][1] - arr[i][0] + 1) {
					sb.append('0');
				} else {
					sb.append('1');
				}
