...
public class TaskA {
	public static void main(String[] args) {
		...
		String[] b = getStringArr(in, m);
		int p = in.nextInt();
		int[] q = getIntArr(in, p);
		String r = solve(n, m, p, a, b, q);
		System.out.println(r);
		in.close();
	}

	private static String solve(int n, int m, int p, String[] a, String[] b, int[] q) {
		StringBuilder sb = new StringBuilder();
		int cycle = n * m;
		for (int qi : q) {
			qi--;
			qi %= cycle;
			sb.append(a[qi % n] + b[qi % m] + "\n");
		}
		return sb.toString();
	}
