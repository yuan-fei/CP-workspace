...
public class ProblemD {
	...
	private static String solve(int n, int[] arr) {
		...
		Arrays.sort(sorted);
		TreeSet<Integer>[] idx = new TreeSet[] { new TreeSet<>(), new TreeSet<>(), new TreeSet<>() };
		for (int i = 0; i < n; i++) {
			idx[arr[i]].add(i);
		}
		...
		for (int i = 0; i < n; i++) {
			while (sorted[i] != arr[i]) {
				int j = idx[arr[i] - 1].last();
				idx[arr[i] - 1].remove(j);
				idx[arr[i] - 1].add(i);
				idx[arr[i]].remove(i);
				idx[arr[i]].add(j);
				swap(arr, i, j);
				steps.add(new int[] { i + 1, j + 1 });
			}
			idx[arr[i]].remove(i);
		}
