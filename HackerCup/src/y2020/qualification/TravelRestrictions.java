...
public class TravelRestrictions {
	public static void main(String[] args) throws FileNotFoundException {
		try (Scanner in = new Scanner(new File("input.txt")); PrintWriter out = new PrintWriter("output.txt")) {
			...
			for (int i = 1; i <= t; i++) {
				int n = in.nextInt();
				String inRules = in.next();
				String outRules = in.next();
				boolean[][] r = solve(n, inRules, outRules);
	...
	private static String str(boolean[][] r) {
	...
	private static boolean[][] solve(int n, String inRules, String outRules) {
		boolean[][] ans = new boolean[n][n];
		for (int i = 0; i < n; i++) {
			ans[i][i] = true;
			for (int j = i + 1; j < n; j++) {
				ans[i][j] = ans[i][j - 1] && (outRules.charAt(j - 1) == 'Y') && (inRules.charAt(j) == 'Y');
			}
			for (int j = i - 1; j >= 0; j--) {
				ans[i][j] = ans[i][j + 1] && (outRules.charAt(j + 1) == 'Y') && (inRules.charAt(j) == 'Y');
			}
