...
public class Timber {
	...
	private static int solve(int n, int[][] a) {
		...
		for (int[] x : a) {
			int p = x[0];
			int h = x[1];
			int l0 = m.getOrDefault(p, 0);
			int l1 = m.getOrDefault(p + h, 0);
			m.put(p + h, Math.max(l1, l0 + h));
			max = Math.max(max, m.get(p + h));
			l0 = m.getOrDefault(p - h, 0);
			l1 = m.getOrDefault(p, 0);
			m.put(p, Math.max(l1, l0 + h));
			max = Math.max(max, m.get(p));
		}
		return max;
	}
