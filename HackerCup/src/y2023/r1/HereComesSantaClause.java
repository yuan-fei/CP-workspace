...
public class HereComesSantaClause {
	...
	private static double solve(int n, int[] a) {
		Arrays.sort(a);
		if (n == 5) {
			return Math.max((double) (a[n - 1] + a[n - 2]) / 2 - (double) (a[2] + a[0]) / 2,
					(double) (a[n - 1] + a[n - 3]) / 2 - (double) (a[1] + a[0]) / 2);
		} else {
			return (double) (a[n - 1] + a[n - 2]) / 2 - (double) (a[1] + a[0]) / 2;
		}
	}
