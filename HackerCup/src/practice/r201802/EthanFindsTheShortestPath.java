...
public class EthanFindsTheShortestPath {
	...
	private static int solve(int n, int k, List<int[]> res) {
		int cnt = Math.min(n, k);
		int ans = 0;
		if (cnt <= 2) {
			res.add(new int[] { 1, n, 1 });
		} else {
			ans = (2 * k - cnt + 1) * cnt / 2 - 2 * k;
			int i = 1;
			for (; i <= cnt - 2; i++) {
				res.add(new int[] { i, i + 1, k - i });
			}
			res.add(new int[] { i, n, k - i });
			res.add(new int[] { 1, n, k });
		}
		return ans;
	}
