...
private fun main() {
    ...
    for (i in 1..n){
        val (a, b ,c) = readlnLongs()
        reqs.add(Req(a, b, c))
    }
    var cnt = reqs.map { it.a }.sum()
    var cost = reqs.map { it.a * it.c }.sum()
    if(cnt > k){
        println(-1)
    }
    else{
...
private data class Req (val a:Long, val b:Long, val c:Long){
    val ext:Long
        get() = b-a
}