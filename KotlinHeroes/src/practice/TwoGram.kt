...
private fun main() {
    val n = readlnInt()
    val s = readln()
    val twoGramCnt = IntArray(26 * 26)

    for (i in 0 until n - 1) {
        twoGramCnt[(s[i] - 'A') * 26 + (s[i + 1] - 'A')]++;
    }
    var maxCnt = 0
    var maxIdx = -1
    for (i in 0 until twoGramCnt.size) {
        if (twoGramCnt[i] > maxCnt) {
            maxCnt = twoGramCnt[i]
            maxIdx = i
        }
    }
    val ret = charArrayOf(('A' + maxIdx / 26), ('A' + maxIdx % 26))
    println(String(ret))
}
